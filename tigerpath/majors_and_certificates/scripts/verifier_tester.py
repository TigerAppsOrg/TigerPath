...
SCHEMA_LOCATION = os.path.join(
    DIR_PATH, "schema.json"
)  # path to the requirements JSON schema
TESTS_LOCATION = os.path.join(
    DIR_PATH, "verifier_tests"
    ...
    for filename in sorted(os.listdir(TESTS_LOCATION)):
        if filename.endswith(".test"):
            ...
            if req_name in [
                "AB",
                "BSE",
            ]:  # checking degree. No validation for degree jsons
                satisfied, courses, req_tree = verifier.check_degree(
                    req_name, courses, year
                )
            elif "Certificate:" in req_name:
                req_name = "".join(req_name.split("Certificate:")[1:])
                major_filename = req_name + "_" + str(year) + ".json"
                major_filepath = os.path.join(
                    DIR_PATH, verifier.CERTIFICATES_LOCATION, major_filename
                )
                ...
                satisfied, courses, req_tree = verifier.check_certificate(
                    req_name, courses, year
                )
            else:  # checking major
                major_filename = req_name + "_" + str(year) + ".json"
                major_filepath = os.path.join(
                    DIR_PATH, verifier.MAJORS_LOCATION, major_filename
                )
                with open(major_filepath, "r", encoding="utf8") as f:
                    requirements = yaml.safe_load(f)
                with open(SCHEMA_LOCATION, "r", encoding="utf8") as s:
                    schema = yaml.safe_load(s)
                jsonschema.validate(requirements, schema)
                satisfied, courses, req_tree = verifier.check_major(
                    req_name, courses, year
                )
            ...
            if not filecmp.cmp(file_path + ".expected", file_path + ".out"):
                print("--- Failed")
                if test_failed == None:
                    test_failed = "echo 'Failed test:' %s; %s %s %s | head -10" % (
                        file_path,
                        diff,
                        file_path + ".expected",
                        file_path + ".out",
