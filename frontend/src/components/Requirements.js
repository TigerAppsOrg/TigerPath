    ...
    $('.reqLabel').each((index, reqLabel) => {
      ...
      reqLabelElement
        ...
        .on('mouseenter', () => {
          ...
          $('body').off('click', '.searchByReq').on('click', '.searchByReq', () => {this.getReqCourses(reqLabelElement.attr('reqpath'))});
        ...
        if ('req_list' in requirement) {
          ...
          return (
            <TreeView key={index} itemClassName={finished} nodeLabel={reqLabel}>
              {requirement['settled'] && requirement['settled'].map((course, index)=>{
                return (
                  <li key={index} className='settled' onClick={(e)=>{this.toggleSettle(course, requirement['path_to'], false)}}>
                    {course}
                  </li>
                );
              })}
              {requirement['unsettled'] && requirement['unsettled'].map((course, index)=>{
                return (
                  <li key={index} className='unsettled text-muted' onClick={(e)=>{this.toggleSettle(course, requirement['path_to'], true)}}>
                    {course} <i className="fa fa-exclamation-circle" title="This course could satisfy multiple requirements. Click to settle it here."></i>
                  </li>
                );
              })}
            </TreeView>
    ...
    return this.props.requirements.map((mainReq, index) => {
      ...
      // major is not supported yet
        ...
        content = (
          <div>
            <p style={{padding: '5px'}}>
              The {name} major is not supported yet. If you would like to request it, let us know <a href="https://goo.gl/forms/pKxjmubIOSCOeR8L2" target="_blank" rel="noopener noreferrer">here</a>.
            </p>
            <p style={{padding: '5px'}}>
      ...
      // render requirements
      let mainReqLabel = (
        <div className='reqLabel' title={'<span>' + name + '</span>'} data-content={popoverContent}>
          <div className='my-arrow root-arrow'></div>
          {name}
        </div>
      );
      return (
        <TreeView key={index} itemClassName={"tree-root " + finished} childrenClassName="tree-sub-reqs" nodeLabel={mainReqLabel}>
          {content}
        </TreeView>
      );
    });
